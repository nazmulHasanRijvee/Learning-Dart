...
main() {
  ...
  double? number1 = double.tryParse(
    stdin.readLineSync()!,
  ); // the variable must be nullable for input,
  ...
  print(
    'sum : ${(number1! + number2!)}',
  ); // null assertion ! tells dart rest assure values will not be null
  // basically converts nullable to non null, only one line needs to be specified other lines automatically understood

  print(
    'sub : ${(number1 - number2).toStringAsFixed(3)}',
  ); // substraction and shows output for 3 decimal digits using toStringAsFixed
